        ...
        {leaders.length === 0 ? (
          ...
          leaders.map((leader) => {
            ...
            return (
              <div 
                key={leader.user_id} 
                className={`relative p-5 border-4 border-black rounded-md transition-transform ${
                  isGold ? 'bg-gradient-to-br from-yellow-200 to-yellow-500' : 'bg-white'
                } ${isTop3 ? 'shadow-brutal' : 'shadow-tile'}`}
              >
                {/* 排名標籤 */}
                <div className="absolute -top-4 -left-2 bg-black text-white w-10 h-10 flex items-center justify-center font-black text-xl border-2 border-white shadow-brutal-sm rotate-[-5deg] rounded-sm">
                  #{leader.rank}
                </div>
                ...
                <div className="flex items-start justify-between ml-8">
                  ...
                  <div className="text-right flex flex-col items-end">
                    <div className="bg-white border-2 border-black px-2 py-1 shadow-brutal-sm mb-2 rounded-sm">
                    ...
                    <div className="text-xs font-bold bg-black text-white px-2 py-1 rounded-sm">
